...
module.exports.loan_item_get = (req, res) => {
    ...
    con.query(`SELECT * FROM loan WHERE item_id = ${req.body.id}`, function (err, result) {
        if (err) {
            console.log(`Query Error Code: ${err.code}`)
            console.log(`Query Error Message: ${err.sqlMessage}`)
            res.status(406).json({ response: 'rejected' })
...
module.exports.item_search = (req, res) => {
    ...
    con.query(`SELECT * FROM item WHERE item_name LIKE "%${req.body.item_name}%"`, function (err, result) {
        if (err) {
            console.log(`Query Error Code: ${err.code}`)
            console.log(`Query Error Message: ${err.sqlMessage}`)
            res.status(406).json({ response: 'rejected' })
...
module.exports.item_search_code = (req, res) => {
    ...
    con.query(`SELECT * FROM item WHERE item_code = '${req.body.code}'`, function (err, result) {
        if (err) {
...
module.exports.item_search_location = (req, res) => {
    con.query(`SELECT * FROM item WHERE item_location LIKE "%${req.body.location}%"`, function (err, result) {
        if (err) {
            console.log(`Query Error Code: ${err.code}`)
            console.log(`Query Error Message: ${err.sqlMessage}`)
            res.status(406).json({ response: 'rejected' })
