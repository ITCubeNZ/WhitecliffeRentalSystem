...
function repopulateDropdowns(incomingData){
  // INTIAL POPULATION BELOW ///////////////////////////////////////////
    ...
    if (incomingData === "reset"){
      ...
      dataSet.apiData.forEach(function (item, index) {   // use original data not data1, data1 information is delayed
        if (item.location === incomingData.location ){  // DONt NOT USE - if (item.location === varLocation){ - IT DELAYS IT, setstate performed after function
  ...
  if (incomingData.location){
    ...
    if (incomingData.location === null && varBooking === null) {
      ...
      dataSet.apiData.forEach(function (item, index) {
        ...
        setData1(result);
    } else if (varLocation === null){  // this for when the location is reset but still need status information
      dataSet.apiData.forEach(function (item, index) {
        if (item.location === incomingData.location ){  // DONt NOT USE - if (item.location === varLocation){ - IT DELAYS IT, setstate performed after function
          result.apiData.push(item);
        }
...
//// status below //////////////////////////////////////////////////////////////////
    ...
    if (incomingData.status === null && varLocation === null) {
      ...
      dataSet.apiData.forEach(function (item, index) {
        if (item.rental_status === incomingData.status ){  // DO NOT USE - if (item.location === varLocation){ - IT DELAYS IT, setstate performed after function
          result.apiData.push(item);  // having the apiData,, wird object array combo
        }
        });
        setData1(result);
      ...
      dataSet.apiData.forEach(function (item, index) {
        if (item.location === varLocation && item.rental_status === incomingData.status){  // DONt NOT USE - if (item.location === varLocation){ - IT DELAYS IT, setstate performed after function
          result.apiData.push(item);
        }
        });
        console.log(result);
 ...
 <div style={styles.box}>
        ...
        <DropdownButton  variant="success" id="dropdown-basic-button" title={dropdownLocation}>
          {varLocationDropdown.map(datas => {
            return (
          <Dropdown.Item key={"arbitary1" + datas} onClick={()=>{repopulate({"location":datas});setdropdownLocation(datas)}}  value={datas}>{datas}</Dropdown.Item>
          )
            })}
          <Dropdown.Divider />
        ...
        {  Object.entries(varStatusDropdown).map(([key, value]) => { 
            return (
          <Dropdown.Item key={"arbitary2" + value} onClick={()=>{repopulate({"status":key});setdropdownBooking("Current Location Status Options")}}  value={key}>{value}</Dropdown.Item>
          )
            })}
          <Dropdown.Divider />
          <Dropdown.Item onClick={() => buttonReset()}>Reset</Dropdown.Item>
        ...
        <Button  variant="success" onClick={() => buttonReset()}>Reset Search</Button>
        ...
        style={{
          ...
          <thead >
            <tr >
              <th style={{backgroundColor: '#212529', color: '#ffffff'}}><b>#</b></th>
              <th style={{backgroundColor: '#212529', color: '#ffffff'}}><b>Item</b></th>
              <th style={{backgroundColor: '#212529', color: '#ffffff'}}><b>Location</b></th>
              <th style={{backgroundColor: '#212529', color: '#ffffff'}}><b>Current Status/To Book</b></th>
            </tr>
          </thead>
          <tbody >
